...
def main():
    """Тестирование точности программы"""
    nlp = stanza.Pipeline('ru', download_method=False, warnings=False)
    # Файл предложений
    with open("datasets/times/main_diapazon.txt", encoding="utf-8") as file:
        things = file.read()

    d = dict(json.loads(things))
    # Куда записываются результаты неверно замененных предложений
    file_wrong = open("wrong.txt", "w", encoding="utf-8")
    # Куда записываются результаты верно замененных предложений
